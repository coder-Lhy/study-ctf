...
![Alt Text](typora-user-images/image-20241206190247587.png)

以此类推，依次判断数据库名第一位

![Alt Text](typora-user-images/image-20241206190347782.png)
...
![Alt Text](typora-user-images/image-20241206192201031.png)

技巧：ascii——会传回第一位的ascii

利用大于号小于号（二分法）

ord是ascii的同名函数，用于ascii被过滤后

![Alt Text](typora-user-images/image-20241206192638506.png)

left()

其是尾字符的变化，所以利用reverse函数将其倒转过来，使其是首字符的变化

substr(str,pos,len)截取从pos开始到最后的所有str字符

pos：是从1开始数的

trim()——用于移除字首字尾的空白的

trim(both/leading/traling)，both首尾都移除,leading首，traling尾

用法：trim(leading I from 'abcd')=trim(leading I+1 from 'abcd')

trim(leading I from 'abcd')=trim(leading I+1 from 'abcd'),类推……

假设第一行若不相等，那么结果就在 I 和 I+1，再利用第二行判断，若第二行相等那么就是 I 
...
> 若被过滤逗号了可以![Alt Text](typora-user-images/image-20241206191202250.png)
